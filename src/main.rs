...
fn derive_master_key(seed: &[u8; 64], network: Network) -> Result<Xpriv, Box<dyn std::error::Error>> {
...
fn create_printable_output(
    ...
    output.push_str(&format!("5. Verify the fingerprint matches: {}\n", fingerprint));
...
fn main() -> Result<(), Box<dyn std::error::Error>> {
    ...
    fs::write(&seed_words_file, mnemonic.words().collect::<Vec<_>>().join("\n"))?;
...
mod tests {
    ...
    fn test_generate_mnemonic() {
        ...
        for word in words {
            ...
            assert!(word.chars().all(|c| c.is_alphabetic()), "Word should contain only letters");
        }
    }
    ...
    fn test_generate_seed() {
        ...
        assert_ne!(seed, seed_with_passphrase, "Seed with passphrase should be different");
    }
    ...
    fn test_get_hardware_wallet_fingerprint() {
        ...
        assert_eq!(fingerprint.len(), 8, "Fingerprint should be 8 hex characters");
    ...
    fn test_create_printable_output() {
        ...
        assert!(output.contains("BITCOIN SEED PHRASE"), "Should contain header");
        assert!(output.contains("Test Wallet"), "Should contain label");
        assert!(output.contains(&fingerprint), "Should contain fingerprint");
        assert!(output.contains("SECURITY WARNING"), "Should contain security warning");
        assert!(output.contains("SEED WORDS"), "Should contain seed words section");
        assert!(output.contains("VERIFICATION CHECKLIST"), "Should contain checklist");
        assert!(output.contains("HARDWARE WALLET IMPORT INSTRUCTIONS"), "Should contain instructions");
        
        // Verify all 24 words are present
        let words: Vec<&str> = mnemonic.words().collect();
        for word in &words {
            assert!(output.contains(word), "Output should contain word: {}", word);
        }
    ...
    fn test_create_simple_word_list() {
        ...
        for (i, word) in words.iter().enumerate() {
            ...
            assert!(output.contains(&expected_line), "Should contain numbered line");
        }
    }
    ...
    fn test_mnemonic_consistency() {
        ...
        assert_eq!(master_key1.to_string(), master_key2.to_string(), 
    ...
    fn test_file_generation() {
        ...
        assert!(!printable_content.is_empty(), "Printable file should not be empty");
        
        let word_list_content = fs::read_to_string(&word_list_file).unwrap();
        assert!(!word_list_content.is_empty(), "Word list file should not be empty");
        
        let seed_words_content = fs::read_to_string(&seed_words_file).unwrap();
        assert!(!seed_words_content.is_empty(), "Seed words file should not be empty");
    ...
    fn test_fingerprint_format() {
        ...
        assert!(fingerprints.len() > 0, "Should generate at least one fingerprint");
    }
}
